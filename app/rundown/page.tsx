...
import { Map, Plus, X, Trash2, CheckCircle2, ChevronDown, ChevronUp, Edit2, CircleDot, Paperclip, Loader2, FileText, CalendarClock } from 'lucide-react';
...
export default function RundownPage() {
  ...
  const defaultPhases = ['H-6 Bulan', 'H-5 Bulan', 'H-4 Bulan', 'H-3 Bulan', 'H-2 Bulan', 'H-1 Bulan', 'Minggu Acara', 'Hari H (Akad & Resepsi)'];
  ...
  const handleSaveTask = async (e: React.FormEvent) => {
    ...
    if (!finalPhase) {
      alert('Fase waktu tidak boleh kosong!');
      return;
    }
    ...
    // Logika Upload File ke Supabase Storage (Bucket: dokumen_rundown)
    if (selectedFile) {
      const fileExt = selectedFile.name.split('.').pop();
      const fileName = `${Date.now()}_${Math.random().toString(36).substring(2)}.${fileExt}`;
      
      const { data, error } = await supabase.storage
        .from('dokumen_rundown')
        .upload(fileName, selectedFile);

      if (error) {
        alert('Gagal upload dokumen: Pastikan nama bucket "dokumen_rundown" sudah dibuat dan public.');
    ...
    if (editingId) {
      const { error } = await supabase.from('rundown').update(payload).eq('id', editingId);
      if (!error) { resetForm(); fetchTasks(); }
    } else {
      const { error } = await supabase.from('rundown').insert([{ ...payload, is_completed: false }]);
      if (!error) { resetForm(); fetchTasks(); }
    }
  ...
  const toggleCompletion = async (id: string, currentStatus: boolean) => {
    ...
    await supabase.from('rundown').update({ is_completed: !currentStatus }).eq('id', id);
    fetchTasks();
  };

  const handleDelete = async (id: string, fileUrl: string | null) => {
    if (!confirm('Hapus agenda rundown ini?')) return;
    
    if (fileUrl) {
      const fileName = fileUrl.split('/').pop();
      if (fileName) await supabase.storage.from('dokumen_rundown').remove([fileName]);
    }
  ...
  const handleDeletePhase = async (phaseName: string) => {
    if (!confirm(`PERINGATAN: Hapus fase rundown "${phaseName}" beserta SELURUH AGENDA di dalamnya?`)) return;
    await supabase.from('rundown').delete().eq('phase', phaseName);
    fetchTasks();
  };
  ...
  if (isLoading) return <div className="flex justify-center pt-20 text-gray-400">Memuat Data Rundown...</div>;
    ...
    <div className="pb-20 max-w-5xl mx-auto">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl md:text-4xl font-serif italic font-semibold mb-2 text-white flex items-center gap-3">
            <CalendarClock size={32} className="text-rose-300" /> Master Rundown
          </h1>
          <p className="text-rose-100 text-sm font-medium">Susun jadwal, timeline persiapan, hingga detail acara di Hari H.</p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm mb-10">
        <div className="flex justify-between items-end mb-3">
          <div>
            <h3 className="font-bold text-[#2C3E50] text-lg">Progres Rundown</h3>
                ...
                <div className={`bg-white rounded-[2rem] border border-l-[8px] transition-all shadow-sm overflow-hidden ${isAllDone ? 'border-emerald-200 border-l-emerald-500 ring-2 ring-emerald-50' : theme.border}`}>
                  ...
                  <div className={`p-6 flex justify-between items-center gap-4 border-b ${isAllDone ? 'bg-emerald-50 border-emerald-100' : `${theme.bg}${theme.border}`}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button onClick={() => handleDeletePhase(phaseName)} className={`p-2 rounded-xl transition ${isAllDone ? 'text-emerald-600 hover:bg-emerald-200' : `${theme.text} opacity-50 hover:opacity-100 hover:bg-white/50`}`} title="Hapus Fase Rundown"><Trash2 size={18} /></button>
                    </div>
                  </div>
      ...
      {/* Modal Tambah/Edit Agenda Rundown */}
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              <h2 className="text-2xl font-serif italic text-rose-900 mb-6">{editingId ? 'Edit Agenda' : 'Tambah Agenda Rundown'}</h2>
              
              <form onSubmit={handleSaveTask} className="space-y-4">
                <div>
                  <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1">Fase / Waktu Pelaksanaan *</label>
                  <select name="phase" value={formData.phase} onChange={handleChange} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-rose-400 bg-gray-50/50" disabled={isUploading}>
                    ...
                    <option value="custom" className="font-bold text-rose-600">+ Tambah Fase/Waktu Baru...</option>
                  </select>
                  
                  <AnimatePresence>
                    {formData.phase === 'custom' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2">
                        <input type="text" placeholder="Cth: Hari H (08:00 - 10:00) Akad..." value={customPhase} onChange={(e) => setCustomPhase(e.target.value)} className="w-full border border-rose-300 rounded-xl p-3 text-sm focus:outline-rose-500 bg-rose-50/50" required disabled={isUploading} />
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div>
                  <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1">Nama Agenda / Kegiatan *</label>
                  <input type="text" name="task_name" value={formData.task_name} onChange={handleChange} placeholder="Cth: Prosesi Akad Nikah" className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-rose-400" required autoFocus disabled={isUploading} />
                </div>

                <div>
                  <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1">Detail / PIC (Opsional)</label>
                  <textarea name="notes" value={formData.notes} onChange={handleChange} rows={2} placeholder="Cth: PIC: WO, MC, Penghulu bersiap..." className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-rose-400 bg-gray-50/50" disabled={isUploading} />
                </div>

                {/* Input File Dokumen */}
                <div className="pt-2">
                  <label className="block text-xs uppercase tracking-wider text-gray-500 mb-2">Lampirkan Dokumen (Opsional)</label>
                  ...
                  <div className="relative border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:bg-gray-50 transition cursor-pointer">
                    ...
                    <div className="flex flex-col items-center justify-center gap-1 text-gray-500">
                      ...
                      <span className="text-xs">Max. 5MB (PDF/JPG/Word)</span>
                    </div>
                  </div>
                </div>
                ...
                <button type="submit" disabled={isUploading} className="w-full bg-rose-900 text-white py-3.5 rounded-xl font-medium hover:bg-rose-950 transition mt-6 flex items-center justify-center gap-2 disabled:bg-rose-900/60 disabled:cursor-not-allowed">
                  {isUploading ? <><Loader2 size={18} className="animate-spin" /> Menyimpan Data...</> : (editingId ? 'Simpan Perubahan' : 'Simpan Agenda')}
                </button>
              </form>
            </motion.div>
          </div>
